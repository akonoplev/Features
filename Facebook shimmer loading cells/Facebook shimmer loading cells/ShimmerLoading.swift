...
public protocol CellsLoadble
{
    func ld_visibleContentViews(unlessIndexe index: Int?)-> [UIView]
}

extension UITableView: CellsLoadble {
    public func ld_visibleContentViews(unlessIndexe index: Int?)-> [UIView] {
        var visiableCell = (self.visibleCells as NSArray).value(forKey: "contentView") as! [UIView]
        if index == nil {
            return visiableCell
        } else {
            visiableCell.remove(at: index!)
        }
        return visiableCell
    }
}
...
open class Loader
{
    ...
    open static func addLoaderTo(_ list : CellsLoadble, unless: Int? )
    {
        self.addLoaderToViews(list.ld_visibleContentViews(unlessIndexe: unless))
    }
    
    
    open static func removeLoaderFrom(_ list : CellsLoadble, unless: Int?)
    {
        self.removeLoaderFromViews(list.ld_visibleContentViews(unlessIndexe: unless))
    }
}
